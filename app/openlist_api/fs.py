...
class FileSystemAPI:
    ...
    def dirs(self, path: str, password: str = "", force_root: bool = False) -> DirsResponse:
        """获取指定目录下的所有子目录。
    ...
    def form_upload(self, file_path: str, file: BinaryIO, as_task: bool = True) -> UploadResponse:
        """表单方式上传文件。
        ...
        response_data = self.client.put("/api/fs/form", files=files, headers=headers)
        return UploadResponse(**response_data)

    def stream_upload(
        self, file_path: str, file_data: BinaryIO, as_task: bool = True
    ) -> UploadResponse | UploadResponseNoTask:
        """流式上传文件。
        ...
        response_data = self.client.put("/api/fs/put", data=file_data, headers=headers)
        return UploadResponse(**response_data) if as_task else UploadResponseNoTask(**response_data)
