...
from app.core.errors import Conflict, LeaseConflict, NotFound, ValidationFailed
...
logger = get_logger("zlabel.app.tasks")
...
class TaskService:
    ...
    def list_tasks(
        ...
        *,
        ...
        """One page of tasks plus the total count.

        ``limit`` is the page size; ``0`` (or ``None``) means "every matching
        task" and is what the desktop's Fetch ▸ "all" asks for. ``total`` is
        always the unpaged count, so a caller can still page through.
        """
        if limit is not None and int(limit) < 0:
            raise ValidationFailed("limit must be >= 0 (0 = every matching task)")
        ...
        with self.db.session_scope() as session:
            ...
            rows_query = self._order(query, order)
            if limit:  # 0 = no LIMIT clause: the caller wants the whole result set
                rows_query = rows_query.limit(int(limit))
            rows = session.scalars(rows_query.offset(offset)).all()
