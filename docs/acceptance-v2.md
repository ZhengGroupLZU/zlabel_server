...
cd zlabel_server && uv run pytest tests/app/test_client_contract.py -q   # 12 项：客户端 ↔ 真实服务端
```

## 1. 两人抢同一任务
- 自动化：`tests/app/test_tasks.py::test_claim_then_conflict_for_the_second_annotator`、
  `tests/app/test_client_contract.py::test_lease_expiry_and_takeover`、
...
## 2. 推理只依赖本次请求的任务
- 自动化：`tests/app/test_inference_worker.py::test_same_task_is_encoded_once`、
  `::test_different_tasks_are_encoded_separately`、`::test_results_follow_the_requested_task`、
  `tests/app/test_predict.py::test_each_predict_sends_its_own_task`
- 手工：同一任务上连续点 3 次（第 2、3 次明显更快，`/metrics` 命中率上升）；交替预测两张不同的图，
  掩膜分别对应各自的图。

## 3. 角色与审计
- 自动化：`tests/app/test_auth.py::test_user_admin_endpoints_are_admin_only`、
  `tests/app/test_tasks.py::test_review_requires_a_reviewer_and_a_note_on_reject`、
...
## 4. 版本冲突
- 自动化：`tests/app/test_annotations.py::test_stale_base_version_conflicts`、
  `::test_force_overwrite_is_reviewer_only`、
  `tests/app/test_client_contract.py::test_annotation_roundtrip_and_optimistic_locking`
- 手工：客户端 A 保存后，客户端 B 用旧版本保存 → 弹"标注冲突"，消息含服务端版本/作者/时间；
  "强制覆盖"仅 reviewer 可见且生效。

## 5. 进度与统计
- 自动化：`tests/app/test_tasks.py::test_progress_reflects_task_states`、
  `tests/app/test_projects.py::test_progress_by_user`、
  `tests/app/test_client_contract.py::test_claim_lease_and_review_roundtrip`（`my-stats`）
- 手工：提交/复核几个任务后，状态栏 `Done: approved/total` 与
  `GET /api/v2/projects/<p>/progress?by_user=true` 一致。

## 6. 推理进程故障隔离
- 自动化：`tests/app/test_predict.py::test_worker_unavailable_is_503`、
  `tests/app/test_inference_worker.py::test_queue_full_is_503`
- 手工：停掉 worker → 点击预测提示"推理不可用，可继续手动标注"，标注/保存/复核/取图全部照常；
  重新启动 worker（API 不用重启）→ 预测恢复。

## 7. 双端版本匹配
- 自动化：`tests/app/test_health.py::test_v1_era_clients_are_told_to_upgrade`（旧客户端得到 410 +
  升级说明）、`tests/app/test_client_contract.py::test_login_carries_user_role_and_capabilities`
- 手工：用旧版客户端连新服务器 → 明确提示升级；用本版客户端连旧服务器 → 登录时报版本不匹配。

## 8. 数据面与迁移
- 自动化：`tests/app/test_migrations.py`（`upgrade head` / `alembic check` 无漂移 / `downgrade base`）
