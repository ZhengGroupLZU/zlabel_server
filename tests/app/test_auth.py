...
from app.db.models import ROLE_ADMIN, ROLE_ANNOTATOR, ROLE_REVIEWER, Session, User, utcnow
...
def test_unknown_token_is_unauthorized_not_stale(client):
    """An unknown token has nothing to renew: ``unauthorized``."""
    resp = client.get(ME, headers={"Authorization": "Bearer who-knows"})
    assert resp.status_code == 401
    assert resp.json()["code"] == "unauthorized"
...
def test_logout_revokes_the_session_immediately(client, auth_headers):
    ...
    assert resp.status_code == 401
    # the token is dead, not unknown: the client should log in again
    assert resp.json()["code"] == "session_stale"
...
def test_user_admin_endpoints_are_admin_only(client, auth_headers, harness):
    ...
    # the role change revokes the old session: dead token -> "log in again"
    stale = client.get(ME, headers=bob)
    assert stale.status_code == 401
    assert stale.json()["code"] == "session_stale"
