...
from app.db.models import Annotation, AuditLog, Task, utcnow
...
def test_list_tasks_limit_zero_returns_everything(client, auth_headers, harness):
    """``limit=0`` is the desktop's Fetch ▸ "all": no page-size ceiling, no 422."""
    files = tuple(f"images/dish01/D{i}.png" for i in range(1, 8))
    headers = bootstrap(client, auth_headers, harness, files=files)

    for limit in (0, 1000):
        body = client.get(
            "/api/v2/projects/projA/tasks",
            params={"limit": limit, "order": "sequence"},
            headers=headers["admin"],
        ).json()
        assert len(body["items"]) == len(files), f"limit={limit}"
        assert body["total"] == len(files)

    # the ceiling still guards a runaway page size, and negatives are refused
    too_big = client.get("/api/v2/projects/projA/tasks", params={"limit": 10001}, headers=headers["admin"])
    assert too_big.status_code == 422
    assert (
        client.get("/api/v2/projects/projA/tasks", params={"limit": -1}, headers=headers["admin"]).status_code
        == 422
    )

    # "everything" still honours offset (a caller may keep paging with limit=0)
    rest = client.get(
        "/api/v2/projects/projA/tasks",
        params={"limit": 0, "offset": len(files) - 2, "order": "sequence"},
        headers=headers["admin"],
    ).json()
    assert len(rest["items"]) == 2

    # the state filter still applies to an unlimited request
    drafts = client.get(
        "/api/v2/projects/projA/tasks",
        params={"limit": 0, "state": "approved"},
        headers=headers["admin"],
    ).json()
