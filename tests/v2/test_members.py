...
@pytest.fixture
def two_users(client, auth_headers, ol):
...
def test_membership_crud(two_users, client, ol):
...
def test_open_mode_keeps_everyone_working(two_users, client, ol):
...
def test_strict_mode_hides_and_blocks_foreign_projects(two_users, client, ol, settings):
    admin, bob, bob_id = two_users
    seed(ol, "projA", files=("a.png",))
    seed(ol, "projB", files=("b.png",))
    client.post("/api/v2/projects/scan", headers=admin)
...
def test_project_reviewer_role_is_scoped(two_users, client, ol, settings, db):
...
def test_membership_needs_a_project_reviewer(two_users, client, ol, settings):
...
def test_unknown_user_or_role_is_rejected(two_users, client, ol):
