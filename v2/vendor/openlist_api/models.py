...
class FileItem(BaseModel):
    ...
    hashinfo: str | None = Field(None, description="hash信息")
    hash_info: Any | None = Field(None, description="hash信息对象")
...
class ListData(BaseModel):
    ...
    def get_dirs(self) -> list[FileItem]:
        """获取所有子目录项。

        Returns:
            list[FileItem]: 所有子目录项列表
        """
        return [item for item in self.content if item.is_dir]

    def get_files(self) -> list[FileItem]:
        """获取所有文件项。

        Returns:
            list[FileItem]: 所有文件项列表
        """
        return [item for item in self.content if not item.is_dir]
...
class FileInfoData(BaseModel):
    ...
    hash_info: Any | None = Field(..., description="hash信息对象")
    raw_url: str = Field(..., description="原始URL")
    readme: str = Field(..., description="README内容")
    header: str = Field(..., description="头部信息")
    provider: str = Field(..., description="存储提供者")
    related: Any | None = Field(..., description="相关信息")
...
class UploadResponseNoTask(BaseResponse):
    """上传文件API响应（无任务信息）。
    ...
    data: None = Field(None, description="null")
...
class StorageInfo(BaseModel):
    ...
    enable_sign: bool | None = Field(None, description="启用签名")
